function signup_alert() {
    ...
    // 빈 칸 확인
    if (username.length + company_name.length + userid.length + company_address.length + password1.length +
            company_tel.length + phone_num.length + email.length + password2.length == 0) {
...
function findpasswd_alert() {
    var userid = document.getElementById("input-id").value;
    var id-check = document.getElementById("id-check").value;
    // 아이디 입력 란이 비워져 있다면
    if (userid.length == 0) {
        alert('아이디를 입력해 주세요.')
